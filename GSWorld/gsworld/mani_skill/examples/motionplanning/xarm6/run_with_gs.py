...
def _main(args, pipeline, proc_id: int = 0, start_seed: int = 0) -> str:
    ...
    # Create an environment instance using Gym's `make()` function.
    # This initializes the ManiSkill environment with detailed control over simulation, rendering, and robot parameters.
    # Below are the arguments explained:
    #
    # - env_id: The string id of the environment, such as "AlignXArmEnv-v1".
    # - robot_uids: Which robot to instantiate in the environment, e.g., "xarm6_uf_gripper".
    # - obs_mode: The type of observation to return (e.g., "rgb+segmentation").
    # - control_mode: How the robot is controlled (e.g., "pd_joint_pos").
    # - render_mode: Render backend mode (e.g., "sensors" means use camera sensors; "rgb_array" for videos).
    # - reward_mode: Specifies the reward type (here, None, i.e., default).
    # - human_render_camera_configs: Visualization settings for the human render camera (here passes the shader).
    # - viewer_camera_configs: Visualization settings for the viewer window (also uses shader configuration).
    # - sim_backend: Simulation backend to use: "auto", "cpu", or "gpu".
    # - parallel_in_single_scene: Whether multiple envs are run together or independently (False is usual for motion planning).
    # - sim_config: A dict for detailed simulation configuration, e.g., frequencies.
    # - max_episode_steps: The max number of steps per episode (sets episode timeout for RL/MP).
    env = gym.make(
        env_id,
        ...
        reward_mode=None,  # Use default reward
        human_render_camera_configs=dict(shader_pack=args.shader),  # Shader pack for human render
        viewer_camera_configs=dict(shader_pack=args.shader),        # Shader pack for viewer window
        sim_backend=args.sim_backend,        # Use user-specified or auto simulation backend
        parallel_in_single_scene=False,      # Disable parallel envs in a single scene for robustness
        sim_config=sim_config,               # Detailed simulation configuration (e.g., freq)
        max_episode_steps=args.ep_len,       # Maximum number of steps before episode terminates
    )
